...
import { highlightText } from '@/app/(root)/company/_components/PublicDocsTab/HighlightText';
import {
  IDocument,
...
const DocsSection = ({
  ...
  const groupedData = useMemo(() => {
    ...
    return data.reduce((acc, doc) => {
      ...
      // Default category is 'Other Attachments' as a fallback
      let category: string = 'Other Attachments';

      // For v3 company documents, categorize as 'Other Attachments' if:
      // - Document is not a Certificate or Incorporation Document
      // - Filename does not contain 'form'
      // This helps separate miscellaneous documents from core company filings
      if (
        version === 'v3' &&
        companyType === 'Company' &&
        doc.documentCategory &&
        doc.documentCategory !== 'Certificates' &&
        doc.documentCategory !== 'Incorporation Documents' &&
        doc.fileName &&
        !doc.fileName.toLowerCase().includes('form ')
      ) {
        category = 'Other Attachments';
      }
      // Check if document is an optional attachment/supplementary file
      // by looking for keywords like 'opt', 'optional', 'attach' in the label
      else if (
        doc.attachmentLabel &&
      ...
      // Special handling for director change documents to use consistent category name
      else if (doc.documentCategory.includes('Change in Directors')) {
        category = 'Change in Directors/Designated Partner';
      }
      // For all other cases, use the original document category as-is
      else {
  ...
  }, [data, version, companyType]); // check for rerender when version changes
  ...
  const filteredData = useMemo(() => {
    ...
    return sortedData.filter((doc) => {
      const searchableFields = [
        ...
        doc.filingDate,
        ...(doc.attachments?.map(
          (attachment: IPublicDocumentAttachment) => attachment.name
        ) || []),
      ];
  ...
  return (
    <Card className={cn('overflow-hidden rounded-lg shadow-md', className)}>
      <div className='flex flex-col lg:flex-row'>
        {/* CATEGORIES SIDEBAR */}
        <div className='w-full flex-shrink-0 bg-muted p-2 lg:w-80 lg:border-r'>
                  ...
                  <Table>
                    ...
                    <TableHeader className='bg-muted/40'>
                      <TableRow className='divide-x'>
                        ...
                        {/* if category is Other Attachments and version is v2, then do not show attachments column */}
                                  ...
                                  doc.attachments.length > 0 ? (
                                    <div className='space-y-1.5'>
                                      ...
                                      <div className='space-y-1.5'>
                                        {doc.attachments.map(
                                          (
                                            attachment: IPublicDocumentAttachment,
                                    ...
                                    <span className='flex items-center gap-1.5 text-sm text-muted-foreground'>
                                      <Paperclip className='h-4 w-4' /> No
                                      Attachments
                                    </span>
                                  )}
                                </TableCell>
