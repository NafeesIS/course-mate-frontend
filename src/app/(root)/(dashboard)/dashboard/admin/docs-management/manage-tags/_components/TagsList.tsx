...
const TagsList: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['tags', page, limit, sort, sortBy, searchQuery],
    queryFn: () =>
      getAllTags({
        page,
        limit,
  ...
  const handleUpdateTag = async (data: TagFormData) => {
    ...
    setIsUpdating(true);
    try {
      await updateTag(selectedTag._id, data.slug, data.name);
  ...
  if (isError) {
    return (
      <div className='flex min-h-[400px] items-center justify-center'>
        <div className='text-lg text-red-600'>Failed to fetch tags.</div>
      </div>
    );
  }
      ...
      <Card>
            ...
            <div>
              ...
              <div className='flex items-center gap-2'>
                <p className='mt-1 hidden text-sm text-gray-600 sm:block'>
                  Manage your {meta?.total || 'All'} Tags
                </p>
                <Button
                  size='icon'
                  variant='outline'
        ...
        <CardContent>
                  ...
                  sortedTags.map((tag) => (
                    <TableRow key={tag._id} className='hover:bg-gray-50'>
                      ...
                      <TableCell className='whitespace-nowrap text-gray-500'>
                        {formatDate(tag.createdAt, 'dd-MMM-yyyy')}
                      </TableCell>
                      <TableCell className='whitespace-nowrap text-gray-500'>
                        {formatDate(tag.updatedAt, 'dd-MMM-yyyy')}
                      </TableCell>
          ...
          {meta && meta.totalPages > 1 && (
            <div className='mt-6'>
              <TagsPagination meta={meta} onPageChange={setPage} />
            </div>
          )}
        </CardContent>
      </Card>
