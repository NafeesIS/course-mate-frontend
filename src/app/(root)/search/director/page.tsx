import FilterBadges from '@/app/(root)/search/_components/director/FilterBadges';
import FilterSection from '@/app/(root)/search/_components/director/FilterSection';
import SearchResults from '@/app/(root)/search/_components/director/SearchResults';
...
export async function generateMetadata({
  ...
  const title = `${searchParams?.query ? `Search: ${searchParams?.query}` : 'Director Search'}`;
  const description = `Search for directors using our advanced search engine. We help you find the right director for your company. Get started today!`;
  // Convert searchParams to a query string
  const params = new URLSearchParams(
    searchParams as Record<string, string>
  ).toString();
  const url = `${BASE_URL_FRONTEND}/search/director${params ? `?${params}` : ''}`;
